...
for d in ds:
    ...
    AudioSegment.from_wav(f'watermarked_audio/{audio_segment_id}_watermarked.wav').set_frame_rate(compressed_sample_rate).export(
        f'watermarked_sample_rate_compressed_audio/{audio_segment_id}_watermarked_compressed_{compressed_sample_rate}.mp3 ', 
        format='mp3', 
        bitrate='128k'
    )

    AudioSealChecker.audioseal_checker(audio_segment_id, compressed_sample_rate, None, None,"sample_rate")
