...
for d in ds:
    ...
    audio = AudioSegment.from_wav(f'watermarked_audio/{audio_segment_id}_watermarked.wav')
    sped_up = audio._spawn(audio.raw_data, overrides={
        "frame_rate": int(audio.frame_rate * speed_factor)
    }).set_frame_rate(audio.frame_rate)  # normalize back to original sample rate

    sped_up.export(
        f'watermarked_speed_compressed_audio/{audio_segment_id}_watermarked_speed_{int(speed_factor*100)}.mp3',
        format='mp3',
        bitrate='128k'
    )

    AudioSealChecker.audioseal_checker(audio_segment_id, None, None, speed_factor,"speed_with_pitch")
