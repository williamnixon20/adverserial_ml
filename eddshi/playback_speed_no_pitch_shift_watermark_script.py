...
for d in ds:
    ...
    y, sr = librosa.load(f'watermarked_audio/{audio_segment_id}_watermarked.wav', sr=None)
    y_stretched = librosa.effects.time_stretch(y, rate=speed_factor)
    sf.write('temp.wav', y_stretched, sr)
    AudioSegment.from_wav('temp.wav').export(
        f'watermarked_speed_no_pitch_compressed_audio/{audio_segment_id}_watermarked_speed_{int(speed_factor*100)}.mp3',
        format='mp3',
        bitrate='128k'
    )

    AudioSealChecker.audioseal_checker(audio_segment_id, None, None, speed_factor,"speed_no_pitch")
